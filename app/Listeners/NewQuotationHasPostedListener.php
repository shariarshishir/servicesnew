...
use App\Mail\NewQuotationPostMailToUser;
use App\Http\Traits\PushNotificationTrait;

class NewQuotationHasPostedListener implements ShouldQueue
{
    ...
    public function handle($event)
    {
        ...
        if($event->user->user_agent == "Dart"){
            Mail::send('emails.new-quotation-to-user', function($message) use($event){
                $message->to($event->user->email);
                $message->subject('New Quotation have been posted on your RFQ.');
            });
        }
        else{
            Mail::to($event->user->email)->send(new NewQuotationPostMailToUser($event->user));
        }
